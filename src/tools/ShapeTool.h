...
// универсальный инструмент для рисования фигур.
// тип фигуры задаётся при создании.
...
public:
    ...
    void onMousePress(QPointF pos, QGraphicsScene *scene) override
    {
        ...
        // создаём временный item нужного типа
        switch (m_shape) {
        case Shape::Rect:
            m_tempItem = scene->addRect(QRectF(pos, pos), pen, brush);
            break;
        case Shape::Ellipse:
            m_tempItem = scene->addEllipse(QRectF(pos, pos), pen, brush);
            break;
        case Shape::Triangle:
            m_tempItem = scene->addPolygon(QPolygonF(), pen, brush);
            break;
        }
    }
    ...
    void onMouseMove(QPointF pos, QGraphicsScene *scene) override
    {
        ...
        switch (m_shape) {
        case Shape::Rect:
            static_cast<QGraphicsRectItem*>(m_tempItem)->setRect(rect);
            break;
        case Shape::Ellipse:
            static_cast<QGraphicsEllipseItem*>(m_tempItem)->setRect(rect);
            break;
        case Shape::Triangle:
            static_cast<QGraphicsPolygonItem*>(m_tempItem)
                ->setPolygon(makeTriangle(rect));
            break;
        }
    }
    ...
    std::shared_ptr<DrawObject> onMouseRelease(QPointF pos, QGraphicsScene *scene) override
    {
        ...
        // если фигура совсем маленькая — не создаём объект
...
private:
    ...
    // normalizedRect — чтобы можно было рисовать в любую сторону
    ...
    // равнобедренный треугольник вписанный в rect
    static QPolygonF makeTriangle(const QRectF &r) {
        QPolygonF poly;
        poly << QPointF(r.left() + r.width() / 2, r.top())  // вершина
             << QPointF(r.left(), r.bottom())                 // нижний левый
             << QPointF(r.right(), r.bottom());               // нижний правый
        return poly;
    }
};
